...
public static class MenuTokenUsuario
{
    ...
    private static void ListarUsuarios(List<ConfiguracaoUsuario> usuarios)
    {
        ...
        for (int i = 0; i < usuarios.Count; i++)
            Paleta.EscreverLinha($"   {i + 1}. {usuarios[i].NomeExibicao}  (token: {MascararToken(usuarios[i].TokenApi)})", Paleta.Info);
    }
    ...
    private static async Task AdicionarUsuarioAsync(List<ConfiguracaoUsuario> usuarios)
    {
        ...
        if (NomeEmUso(usuarios, nome, ignorar: null))
        {
        ...
        string chave = GerarChaveUnica(nome, usuarios);
    ...
    private static async Task EditarUsuarioAsync(List<ConfiguracaoUsuario> usuarios)
    {
        ...
        if (!string.IsNullOrWhiteSpace(nome))
        {
            if (NomeEmUso(usuarios, nome, ignorar: usuario))
    ...
    private static bool NomeEmUso(List<ConfiguracaoUsuario> usuarios, string nome, ConfiguracaoUsuario? ignorar)
        => usuarios.Any(u => u != ignorar && u.NomeExibicao.Equals(nome, StringComparison.OrdinalIgnoreCase));

    private static string GerarChaveUnica(string nome, List<ConfiguracaoUsuario> usuariosExistentes)
    {
