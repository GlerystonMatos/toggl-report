...
public static class EscritorRelatorioConsole
{
    ...
    {
        Console.WriteLine();
        Paleta.EscreverLinha(Tela.LinhaPreenchida($" ═══ {nomeExibicao} ({Rotulos.Data(de)} a {Rotulos.Data(ate)}) ", '═'), Paleta.Titulo);
        ...
        if (agrupamento is "tag" or "ambos")
            ImprimirSecaoTag(ServicoAgrupamento.AgruparPorTagFiltrada(registros, tagsDetalhadas));
    ...
    private static void ImprimirSecaoTag(List<KeyValuePair<string, long>> exibidas)
    {
    ...
    private static void ImprimirEmAndamento(List<RegistroTempoDto> emAndamento)
    {
        ...
        foreach (RegistroTempoDto registro in emAndamento)
        {
            ...
            string textoInicio = registro.Inicio is DateTimeOffset inicio
                ? $"{inicio.ToLocalTime():HH:mm} do dia {Rotulos.Data(inicio.ToLocalTime().DateTime)}"
