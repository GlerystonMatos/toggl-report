...
| `GET/PUT` | `/api/sprint/status-final` | Mapeamento global de status do Jira usados nos totalizadores Pendentes/Concluídas do **cabeçalho** do Sprint (`{ statusConcluido, statusIgnorado: string[] }`), desde 2026-09-17 — não usados no totalizador por colaborador (ver `LinhaColaboradorSprint` acima). `PUT` normaliza as listas (trim + distinct case-insensitive) e devolve 400 se o mesmo status aparecer nas duas. Seção `[SprintStatusFinal]` de `ConfiguracoesGerais.ini`; exibidas só leitura na etapa Consultar (frontend), desde 2026-09-19 — a 1ª versão (2026-09-18) tinha colocado essa exibição na tela de Acompanhamento por engano |
...
- **Colaboradores** (`ResultadoSprint.Colaboradores`): uma linha por colaborador selecionado — `record LinhaColaboradorSprint(NomeExibicao, Sigla, Cor, Td, SegundosRealizados, TarefasPendentes, TarefasConcluidas)`. `TarefasPendentes`/`TarefasConcluidas` **não** usam `ContarPendentesEConcluidas`/`[SprintStatusFinal]` (corrigido em 2026-09-18) — contam, por **grupo** (DEV/REV/QA) que o colaborador ocupa numa linha de descrição, a mesma classificação Pendente/Concluído da coluna "Situação" da grid (`GrupoResponsavelStatus`/`SituacaoSemGrupoResponsavel`, ver seção Jira abaixo); um colaborador em dois grupos da mesma linha pode contar uma vez em cada coluna, com resultados diferentes. Antes contava por descrição inteira, ignorando o grupo — bug: colaboradores em grupos diferentes da mesma linha sempre contavam igual. `Td` é igual para todos; `SegundosRealizados` = soma das durações `>= 0` de **todos** os apontamentos do usuário no cache do sprint (com ou sem tag de categoria). Colaborador sem nenhum apontamento entra zerado. Sem detalhamento por dia — isso é o Gant. A tabela de colaboradores do frontend mostra **Colab. (nome completo) · sigla · Tempo por colaborador (`Td`, ex-"Total") · Realizado · Disponível · Pendentes · Concluídas** com rodapé somando Pendentes/Concluídas — "Disponível" = `Tempo por colaborador − Realizado` é calculada no frontend (verde quando positiva, vermelha quando negativa, neutra em zero); desde 2026-09-08 as colunas **Disponível, Pendentes e Concluídas** ficam **sempre em negrito**, mesmo neutras/zeradas (antes só "Disponível" tinha negrito condicional).
